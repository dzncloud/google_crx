...
import com.haici.googlecrx.utils.callback.HttpCallback;
...
public class HttpClientHelper implements HttpHelper {
    ...
    protected HttpMethod getGetMethod(String uri, String queryStr, String charset, Header[] headers) {
    ...
    protected <T> GetMethod setParameter(GetMethod getMethod, Map<String, T> data, String charset) {
        ...
        if (data != null && data.size() != 0) {
            List<NameValuePair> paras = new ArrayList<>(data.size());
            ...
            if (CollectionUtils.isNotEmpty(paras)) {
                ...
                if (qStr != null) {
                    getMethod.setQueryString(qStr + "&" + EncodingUtil.formUrlEncode(paras.toArray(new NameValuePair[paras.size()]), charset));
                } else {
        ...
        if (data != null && data.size() > 0) {
            for (Entry<String, T> entry : data.entrySet()) {
                ...
                if (value != null) {
                    ...
                    } else if (value instanceof MultipartFile) { // 增加上传文件处理
                        try {
                            logger.info("=========2 key ======" + entry.getKey());
                            MultipartFile mFile = (MultipartFile) value;
                            File file = new File(FileUtils.getTempDirectory().getAbsolutePath() + "/" + mFile.getOriginalFilename());
                            logger.info("file : " + file.getPath());

                            FileUtils.writeByteArrayToFile(file, mFile.getBytes());
                            FilePart filePart = new FilePart(entry.getKey(), file);
                            MultipartRequestEntity entity = new MultipartRequestEntity(new Part[] { filePart }, postMethod.getParams());
                            postMethod.setRequestEntity(entity);

                            return postMethod;
                        } catch (Exception e) {

                            logger.info("setParameter ", e);
                        }
                    } else {
     ...
     */
    @Override
    public <T> int get(String address, Map<String, T> data, String charset, Cookie[] cookies, Header[] headers, HttpCallback callback) throws IOException {
     ...
     */
    @Override
    public <T> int get(String address, Map<String, T> data, String charset, Map<String, String> cookies, Map<String, String> headers, HttpCallback callback) throws IOException {
        return this.exec(this.getGetMethod(address, data, charset, headers), this.setHttpState(null, cookies, address), callback);
    }

    @Override
    public int get(String address, String queryStr, String charset, Map<String, String> cookies, Header[] headers, HttpCallback callback) throws IOException {
        return this.exec(this.getGetMethod(address, queryStr, charset, headers), this.setHttpState(null, cookies, address), callback);
    }
     ...
     */
    @Override
    public <T> int post(String address, Map<String, T> data, String charset, Map<String, String> cookies, Map<String, String> headers, HttpCallback callback) throws IOException {
        return this.exec(this.getPostMethod(address, charset, headers, data), this.setHttpState(null, cookies, address), callback);
    }
     ...
     */
    @Override
    public <T> int post(String address, Map<String, T> data, String charset, Cookie[] cookies, Header[] headers, HttpCallback callback) throws IOException {
        return this.exec(this.getPostMethod(address, charset, headers, data), this.setHttpState(null, cookies, address), callback);
    }
     ...
     */
    @Override
    public int post(String address, InputStream data, String charset, Cookie[] cookies, Header[] headers, HttpCallback callback) throws IOException {
        return this.exec(this.getPostMethod(address, charset, headers, data), this.setHttpState(null, cookies, address), callback);
    }
     ...
     */
    @Override
    public int post(String address, InputStream data, String charset, Map<String, String> cookies, Map<String, String> headers, HttpCallback callback) throws IOException {
        return this.exec(this.getPostMethod(address, charset, headers, data), this.setHttpState(null, cookies, address), callback);
    }

    @Override
    public int post(String address, byte[] data, String charset, Map<String, String> cookies, Header[] headers, HttpCallback callback) throws IOException {
        return this.exec(this.getPostMethod(address, charset, headers, new ByteArrayInputStream(data)), this.setHttpState(null, cookies, address), callback);
    }
    ...
    @Override
    public <T> int put(String address, Map<String, T> data, String charset, Cookie[] cookies, Header[] headers, HttpCallback callback) throws IOException {
        return this.exec(this.getPutMethod(address, charset, headers, data), this.setHttpState(null, cookies, address), callback);
    }

    @Override
    public int put(String address, InputStream data, String charset, Cookie[] cookies, Header[] headers, HttpCallback callback) throws IOException {
        return this.exec(this.getPutMethod(address, charset, headers, data), this.setHttpState(null, cookies, address), callback);
    }

    @Override
    public int put(String address, byte[] data, String charset, Cookie[] cookies, Header[] headers, HttpCallback callback) throws IOException {
        return this.exec(this.getPutMethod(address, charset, headers, data), this.setHttpState(null, cookies, address), callback);
    }
    ...
    public int put(String address, String data, String charset, String contentType, HttpCallback callback) throws IOException {
        ...
        return this.exec(this.getPutMethod(address, charset, headers, contentType, data), this.setHttpState(null, (Cookie[]) null, address), callback);
    }
    ...
    protected <T> HttpMethod getPutMethod(String uri, String charset, Header[] headers, Map<String, T> data) throws UnsupportedEncodingException {
        return this.setEntity(this.getPutMethod(uri, charset, headers, HttpClientHelper.DEFAULT_POST_CONTENT_TYPE), data, charset);
    }

    protected HttpMethod getPutMethod(String uri, String charset, Header[] headers, byte[] data) throws UnsupportedEncodingException {
        return this.setEntity(this.getPutMethod(uri, charset, headers, HttpClientHelper.DEFAULT_POST_CONTENT_TYPE), data);
    }

    protected HttpMethod getPutMethod(String uri, String charset, Header[] headers, String contentType, String data) throws UnsupportedEncodingException {
    ...
    protected PutMethod setEntity(PutMethod putMethod, String data, String contentType, String charset) throws UnsupportedEncodingException {
        if (data != null) {
    ...
    protected <T> PutMethod setEntity(PutMethod putMethod, Map<String, T> data, String charset) throws UnsupportedEncodingException {
        if (data != null && data.size() > 0) {
            ...
            for (Entry<String, T> entry : data.entrySet()) {
                ...
                if (value != null) {
                    if (value instanceof String) {
                        builder.append(URLEncoder.encode(entry.getKey(), charset)).append("=").append(URLEncoder.encode((String) value, charset)).append("&");
                    } else if (value instanceof Iterable) {
                        ...
                        builder.append(URLEncoder.encode(entry.getKey(), charset)).append("=").append(URLEncoder.encode(String.valueOf(value), charset)).append("&");
                    }
                }
            }
        }
