...
public class ResponseBodyCallback implements HttpCallback {
    ...
    /**
     * 
     * @param charset 指定返回結果編碼
     * @param httpstaus4collect 当返回的状态码等于该值时采集response数据
     */
     ...
     */
    @Override
    public void callback(int httpstaus, HttpMethod httpMethod, Cookie[] cookies) throws IOException {
        this.httpStatus = httpstaus;
        if (this.httpstaus4collect == null || this.httpstaus4collect.intValue() == httpstaus) {
            if (this.charset == null) {
                this.body = httpMethod.getResponseBodyAsString();
            } else {
                this.body = new String(httpMethod.getResponseBody(), this.charset);
            }
            this.bytes = httpMethod.getResponseBody();
        }
    }
